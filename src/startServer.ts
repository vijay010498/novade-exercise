...
const startServer = async () => {
  if (cluster.isMaster) {
    //first time when the sever start's will be master which will not process any request, act as a master --- added on 22/05/21
    ...
    await app.listen(PORT, () => {
      console.log(`Worker with process id ${process.pid} started on 5000`); //message printed on console to know that the server has started
    });
  }
  try {
